...
const authenticate = async (username,password) => {
    try {
     return await axios.post("http://register.ecellmnnit.in:3001/checkuser", { username,password });
    } catch ({ message }) {
...
var main = async () =>{
    prompt.start();
    prompt.message = '';
    const {username,password } = await get (prompt_attributes);
    ...
    if(res.data.stat==true){
        ...
        spinner.fail('Authentication Failed Retry...')
        main();
    }    
}
